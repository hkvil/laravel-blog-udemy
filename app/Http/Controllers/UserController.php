...
class UserController extends Controller
{
    ...
    private function getSharedData($user){
        ...
        if (auth()->check()) {
            ...
            ->where('followeduser', $user->id)->count();
        }
    ...
    public function storeAvatar(Request $request){
        ...
        $user = auth()->user();
        $randomFileName = $user->id . '-' . uniqid() . '.jpg';

        $imgData = Image::make($request->file('avatar'))->resize(120, 120)->encode('jpg');
        Storage::put('public/avatars/' . $randomFileName, $imgData);

        $oldAvatar = $user->avatar;

        $user->avatar = $randomFileName;
        // @phpstan-ignore-next-line,abaikan saja
        $user->save();
